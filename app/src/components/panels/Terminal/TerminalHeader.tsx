...
export const TerminalHeader = memo(({ projectId, onDragStart, onRemove }: { projectId: string, onDragStart?: (e: React.PointerEvent) => void, onRemove?: () => void }) => {
  ...
  if (!p) return <div className="h-8 bg-sidebar-bg/50 border-b border-border shrink-0" />;
  ...
  return (
    <div onPointerDown={(e: React.PointerEvent) => onDragStart?.(e)} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}
      className={`h-8 px-3 flex items-center bg-sidebar-bg/50 border-b border-border shrink-0 justify-between overflow-hidden group/header ${onDragStart ? 'cursor-grab active:cursor-grabbing' : ''}`}
    >
