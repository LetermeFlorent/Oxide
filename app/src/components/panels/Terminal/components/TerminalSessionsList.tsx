...
export const TerminalSessionsList = ({ sessions, activeId, projectId, switchSession, closeSession, addSession }: any) => {
  ...
  const renameSession = useStore(s => s.renameTerminalSession);
      ...
      {sessions.map((s: any) => (
        <div key={s.id} className="flex items-center h-full group relative shrink-0">
          <TerminalTab 
            ...
            onClick={() => switchSession(projectId, s.id)} 
            onDoubleClick={() => setPromptModal({
              show: true,
              title: "Rename Terminal",
              label: "Enter new name for this terminal session",
              defaultValue: s.name,
              onConfirm: (newName) => renameSession(projectId, s.id, newName)
            })}
            icon={TerminalIcon} 
            label={s.name || 'Bash'} 
            activeClass="text-foreground border-black" 
          />
          {sessions.length > 1 && (
            <button type="button" onClick={(e) => { e.stopPropagation(); closeSession(projectId, s.id); }} className={`ml-1 p-0.5 rounded-full hover:bg-gray-200 text-foreground/40 hover:text-red-500 transition-all ${activeId === s.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}><X size={8} /></button>
          )}
          <div className="w-2" />
        </div>
      ))}
