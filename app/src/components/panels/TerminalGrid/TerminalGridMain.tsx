...
export const TerminalGrid = memo(({ overviewId }: { overviewId: string }) => {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-panel-bg overflow-hidden rounded-xl border border-border shadow-sm">
      <GridHeader name={overview?.name || ""} count={n} cmd={masterCmd} setCmd={setMasterCmd} onBroadcast={handleBroadcast} />
      <div ref={containerRef} className="flex-1 relative min-h-0 bg-sidebar-bg/30 p-2">
        {localOrder.map((id, index) => (
          <GridItem key={id} id={id} overviewId={overviewId} onRemove={handleRemove} onDragStart={() => onDragStart(id)} onDrag={(info: any) => handleDrag(id, info)} onDragEnd={onDragEnd} containerRef={containerRef} style={{ position: 'absolute', top: `${(Math.floor(index / cols) / rows) * 100}%`, left: `${((index % cols) / cols) * 100}%`, width: `${100 / cols}%`, height: `${100 / rows}%`, padding: '4px' }} />
        ))}
      </div>
    </div>
  );
