...
export const ResetConfirmModal = memo(({ onHide }: { onHide: () => void }) => (
  <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/50 animate-in fade-in">
    <div className="w-full max-w-sm bg-panel-bg border border-border rounded-3xl shadow-2xl p-8 space-y-6">
      <div className="w-16 h-16 bg-red-500/10 rounded-2xl flex items-center justify-center mx-auto mb-4"><AlertTriangle size={32} className="text-red-500" /></div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <button onClick={onHide} className="w-full py-4 bg-sidebar-bg text-foreground/60 hover:bg-hover-bg text-[10px] font-black uppercase tracking-widest rounded-2xl transition-all">Abort Protocol</button>
      </div>
    </div>
  </div>
));
