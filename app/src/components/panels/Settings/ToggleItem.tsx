...
export const ToggleItem = memo(({ icon: Icon, label, active, onClick }: any) => (
  ...
  >
    <div className="flex items-center gap-3">
      <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${active ? 'bg-foreground text-background' : 'bg-sidebar-bg text-foreground/40'}`}>
        <Icon size={14} />
      </div>
      <span className="text-[10px] font-black uppercase tracking-tight text-foreground/60">{label}</span>
    </div>
    <div className={`w-8 h-4 rounded-full relative ${active ? 'bg-foreground' : 'bg-active-bg'}`}>
