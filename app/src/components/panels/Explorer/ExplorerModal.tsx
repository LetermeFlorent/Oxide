    ...
    <AnimatePresence><div className="fixed inset-0 z-[2000] flex items-center justify-center p-6 bg-black/10 backdrop-blur-sm">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-xs bg-white border border-gray-100 rounded-[2rem] shadow-2xl p-6">
        ...
        <div className="flex gap-2">
          ...
          <button disabled={!name.trim()} onClick={() => onConfirm(name.trim())} className="flex-1 py-3.5 bg-black hover:bg-gray-800 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 shadow-lg shadow-black/10 disabled:opacity-30 disabled:pointer-events-none">{t('common.create')}</button>
        </div>
      </motion.div>
    </div></AnimatePresence>
  );
