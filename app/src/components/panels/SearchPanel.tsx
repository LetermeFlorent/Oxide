...
export const SearchPanel = ({ onFileClick }: { onFileClick: (f: any) => void }) => {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-sidebar-bg select-none">
      <div className="p-3 border-b border-border flex flex-col gap-2">
        <div className="relative">
          <Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-foreground/40" />
          <input autoFocus type="text" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} placeholder="Search..." className="w-full pl-8 pr-8 py-1.5 bg-panel-bg border border-border rounded-lg text-[11px] font-bold outline-none focus:border-foreground/30 text-foreground" />
          {loading && <Loader2 size={10} className="absolute right-3 top-1/2 -translate-y-1/2 animate-spin text-foreground/40" />}
        </div>
      </div>
...
const SearchResultGroup = ({ group, onFileClick }: any) => {
  ...
  return (
    <div className="flex flex-col">
      <div onClick={() => setExpanded(!expanded)} className="flex items-center gap-1.5 px-2 py-1 hover:bg-hover-bg rounded cursor-pointer transition-colors">
      ...
      {expanded && group.items.map((item: any, i: number) => (
        <div key={i} onClick={() => onFileClick({ path: item.path, name: fileName, line: item.line })} className="pl-6 pr-2 py-1 hover:bg-hover-bg rounded cursor-pointer group flex flex-col gap-0.5">
