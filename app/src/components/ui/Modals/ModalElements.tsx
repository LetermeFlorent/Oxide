...
export const ModalButton = memo(({ onClick, label, variant = 'secondary', disabled = false, className = "" }: any) => {
  const base = "flex-1 py-3.5 rounded-[12px] text-[10px] font-black uppercase tracking-widest transition-all active:scale-95";
  const styles: any = {
...
export const ModalInput = memo(({ value, onChange, onKeyDown, autoFocus = false }: any) => (
  <input 
    ...
    onKeyDown={onKeyDown}
    className="w-full px-4 py-3.5 bg-sidebar-bg border border-border rounded-[12px] text-[12px] font-bold text-gray-700 outline-none focus:border-black/20 focus:bg-panel-bg transition-all mb-5" 
  />
));
