...
export const ProjectTabs = memo(({ onOpen }: { onOpen: () => void }) => {
  ...
  return (
    <div ref={tabBarRef} onWheel={(e) => !verticalTabs && (e.currentTarget.scrollLeft += e.deltaY)} data-tauri-drag-region className={`flex shrink-0 select-none transition-colors relative ${isDragging ? 'z-[9999] overflow-visible' : 'z-[1000]'} ${verticalTabs ? 'flex-col gap-1 p-2 w-full h-full overflow-y-auto scrollbar-modern-thin' : `items-center overflow-x-auto scrollbar-modern-thin ${compactMode ? 'gap-0 h-8 border-b border-border bg-panel-bg' : 'gap-1 h-10 pb-1 -mx-1 px-1'}`}`}>
      <AnimatePresence>
      ...
      <div className={`flex items-center shrink-0 ${verticalTabs ? 'w-full py-4 border-t border-border mt-2 justify-center gap-4' : `px-2 ${compactMode ? 'h-full border-l border-border bg-panel-bg' : 'gap-1 ml-1'}`}`}>
