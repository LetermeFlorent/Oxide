          ...
          {projects.filter((p: any) => p && p.id && p.id.trim() !== "").map((p: any, idx: number) => (
            <button key={safeKey('edit-ov', p.id, idx)} onClick={() => toggleProject(p.id)} className={`w-full flex items-center justify-between px-4 py-3 rounded-2xl transition-all ${selectedIds.includes(p.id) ? 'bg-gray-50 border border-gray-100' : 'hover:bg-gray-50 border border-transparent'}`}>
              <div className="flex items-center gap-3">
                <div className={`p-2 rounded-xl ${selectedIds.includes(p.id) ? 'bg-black text-white' : 'bg-gray-100 text-gray-400'}`}>
                  <Terminal size={14} />
                </div>
                <div className="flex flex-col items-start">
                  <span className="text-[10px] font-bold text-gray-600 truncate max-w-[180px]">{p.name}</span>
                  <span className="text-[8px] font-medium text-gray-400 uppercase tracking-widest">Project Terminal</span>
                </div>
              </div>
              <div className={`w-5 h-5 rounded-lg flex items-center justify-center border transition-all ${selectedIds.includes(p.id) ? 'bg-black border-black text-white' : 'bg-white border-gray-200'}`}>
