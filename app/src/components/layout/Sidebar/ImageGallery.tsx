  ...
  return (
    <div className={`flex flex-col gap-2 ${compactMode ? 'p-1 bg-sidebar-bg/50' : 'p-3 bg-panel-bg border border-border shadow-sm rounded-2xl'}`}>
      <div className="flex items-center justify-between px-1">
        ...
        {hasMore && <MoreHorizontal size={10} className="text-foreground/30" />}
      </div>
      <div className="grid grid-cols-4 gap-1.5">
        {displayImages.map((img: any, idx: number) => (
          <div key={safeKey('img', img.path, idx)} onClick={() => onFileClick(img)} className="aspect-square group relative rounded-lg overflow-hidden cursor-pointer hover:ring-2 hover:ring-foreground transition-all">
