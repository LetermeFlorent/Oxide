...
pub fn scan_dir_streaming(app: &AppHandle, event_id: &str, dir_path: &str, images: &mut Vec<FileNode>, recursive: bool, depth: u32) -> Vec<FileNode> {
    ...
    if let Ok(entries) = fs::read_dir(dir_path) {
        ...
        for entry in entries.flatten() {
            ...
            if !is_dir {
                ...
                if [".png", ".jpg", ".jpeg", ".gif", ".webp", ".svg"].iter().any(|&e| lower.ends_with(e)) {
                    images.push(FileNode { name: name.clone(), path: path_str.clone(), is_folder: false, children: None });
                }
            }

            let children = if is_dir {
                if recursive && depth < 3 { Some(scan_dir_streaming(app, event_id, &path_str, images, true, depth + 1)) }
                else { Some(vec![]) }
            } else { None };
    ...
    nodes.sort_by(|a, b| if a.is_folder == b.is_folder { a.name.cmp(&b.name) } else if a.is_folder { std::cmp::Ordering::Less } else { std::cmp::Ordering::Greater });
    nodes
}
...
pub fn scan_dir(dir_path: &str, images: &mut Vec<FileNode>, recursive: bool, depth: u32) -> Vec<FileNode> {
    ...
    if let Ok(entries) = fs::read_dir(dir_path) {
        ...
        for entry in entries.flatten() {
            ...
            let children = if is_dir {
                if recursive && depth < 3 { Some(scan_dir(&path_str, images, true, depth + 1)) }
                else { Some(vec![]) }
            } else { None };
