...
pub async fn list_folder_lsm(
    ...
    let lsm = state
        ...
        .ok_or("LSM DB not initialized")?;
    let (m, h) = (
        lsm.open_tree("meta").unwrap(),
        lsm.open_tree("hierarchy").unwrap(),
    );
    ...
    Ok(ps
        ...
        .filter_map(|p| {
            m.get(p)
                .unwrap()
                .map(|v| decode_from_slice(&v, config::standard()).unwrap().0)
        })
        .collect())
}
...
fn store_node(meta: &sled::Tree, hier: &sled::Tree, n: FileNode) -> Result<(), String> {
    ...
    meta.insert(&path, encode_to_vec(&n, config::standard()).unwrap())
        .unwrap();
    if let Some(p) = Path::new(&path).parent().and_then(|p| p.to_str()) {
        let mut c: Vec<String> = hier
            .get(p)
            .unwrap()
            .map(|v| decode_from_slice(&v, config::standard()).unwrap().0)
            .unwrap_or_default();
        if !c.contains(&path) {
            c.push(path.clone());
            hier.insert(p, encode_to_vec(&c, config::standard()).unwrap())
                .unwrap();
        }
    }
    Ok(())
...
fn flatten_tree(nodes: Vec<FileNode>) -> Vec<FileNode> {
    ...
    for mut n in nodes {
        let c = n.children.take();
        flat.push(n);
        if let Some(list) = c {
            flat.extend(flatten_tree(list));
        }
    }
    flat
}
