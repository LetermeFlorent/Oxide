...
pub fn get_children_from_db(db: Arc<Mutex<rusqlite::Connection>>, parent_path: &str) -> Result<Vec<FileNode>> {
    ...
    let rows = stmt.query_map(params![parent_path], |row| {
        ...
        Ok(FileNode { name: row.get(0)?, path: row.get(1)?, is_folder: is_dir, children: if is_dir { Some(vec![]) } else { None } })
    })?;
    let mut nodes = Vec::new();
    for node in rows.flatten() { nodes.push(node); }
    Ok(nodes)
}

#[tauri::command]
pub async fn list_folder_from_db(state: tauri::State<'_, AppState>, path: String) -> Result<Vec<FileNode>, String> {
    get_children_from_db(state.db.clone(), &path).map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn index_project_db(state: tauri::State<'_, AppState>, path: String) -> Result<(), String> {
