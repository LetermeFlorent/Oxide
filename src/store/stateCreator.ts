import { ProjectStatus, ViewMode } from "./types";
...
export const stateCreator = (set: any) => ({
  ...initialState,
  ...groupActions(set),
  setHydrated: (h: boolean) => set({ hydrated: h }),
  
  // Project Management
  addProject: (pa: string, n: string, t: any) => set(ProjectManagementActions.addProject(pa, n, t)),
  addProjects: (it: any[]) => set(ProjectManagementActions.addProjects(it)),
  switchProject: (id: string) => set((s: any) => ({ 
    activeProjectId: id, 
    showSettings: id === 'settings' ? true : s.showSettings 
  })),
  closeProject: (id: string) => set(ProjectManagementActions.closeProject(id)),
  ...
  // Tree & Sync
  updateProjectTree: (id: string, t: any, i: any) => set(TreeActions.updateProjectTree(id, t, i)),
  applyFilePatch: (projectId: string, patch: any) => set(TreeActions.applyFilePatch(projectId, patch)),
  setProjectStatus: (id: string, st: ProjectStatus) => set(ProjectManagementActions.setProjectStatus(id, st)),
  setFollowedFile: (pid: string, pa: any, pr: any) => set(ProjectManagementActions.setFollowedFile(pid, pa, pr)),
  
  // UI Actions
  setViewMode: (v: ViewMode) => set(UIActions.setViewMode(v)),
  toggleExplorer: () => set(UIActions.toggleExplorer()),
  togglePreview: () => set(UIActions.togglePreview()),
  toggleSettings: (sh?: boolean) => set(UIActions.toggleSettings(sh)),
  setSetting: (k: string, v: boolean) => set(UIActions.setSetting(k, v)),
  ...
  toggleFolder: (p: string) => set((s: any) => TreeActions.toggleFolder(p, s.activeProjectId, monitoredInvoke)(s)),
